<?php
  ...
  $sess = session_id();
  if(!$sess) {
    header("Location: ../../../SqlMap-Testing");
  }
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------> 
             <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">The results of your testing are as follows, <?php echo $userRow['userName']; ?></h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
<!------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 <div class="col-lg-12">
   <div class="panel panel-default">
      <div class="panel-heading">
        ...
        <div class="panel-body sizeofbody">
      <div class="row">
        ...
        <div class="col-md-8">
          <div class="form-group">
            ...
            <label for="results_textarea">If you only see logs, it means that eather the tool couldn't find any injections points or an Error occoured.</label>

            <?php
              if(sizeof($scanData['data']) > 0) {                       // Scan Data, present to user as nice as we can :)
                echo '<textarea class="form-control" id="results_summary_data_textarea" rows="40">';
                $displayed=false;
                $dt = new DateTime();
            ...
            <div id="display_scan_info_textarea" align="central">
              <br /><br />
              <label for="scan_info_textarea">Log Scan Details</label>
              <textarea class="form-control" id="scan_info_textarea" rows="20" style="max-height:486px; max-width:987px; min-height:486px; min-width:987px;">
              <?php
                echo "\n";
            ...
            <?php
              ...
              if(sizeof($scanData['data']) > 0) {
                echo '<div id="display_scan_data_textarea" align="central">';
                echo '  <br /><br />';
    ...
    <?php  } else { // No API, Can't Do Anything.... ?>
      <div class="epic_fail" align="center">
        <p style="font-size:26px">Epic Failure!</p><br />
